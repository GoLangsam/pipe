...
func (t *Traffic) Feed(urls []*url.URL, parent *url.URL, depth int) {
	...
	t.once.Do(func() {
		go func(t *Traffic) {
			t.done <- <-t.SiteDoneWait(t.sites, t.wg)
			close(t.done)
		}(t)
	})
}
...
func (t *Traffic) Processor(crawl func(s Site), parallel int) (done <-chan struct{}) {
	...
	sites, seen := t.SiteForkSeenAttr(t.SitePipeEnter(t.sites, t.wg), Site.attr)
	for _, sites := range t.SiteStrew(t.SitePipeAdjust(sites), parallel) {
