...
import (
	...
	"github.com/GoLangsam/pipe/examples/httpsyet/v6"
)
...
// ---------------------------------------------------------------------------
// Helpers to see which pages have been visited how often.

type seen struct {
	names      map[string]int // seen counter
	sync.Mutex                // guard
}
...
func (visited *seen) new(name string) {
	visited.Lock()
	visited.names[name] = 0
	visited.Unlock()
}

func (visited *seen) add(name string) {
	visited.Lock()
	visited.names[name]++
	visited.Unlock()
}
