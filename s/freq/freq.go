...
// upon close.
func anyThingDoneFreq(inp <-chan anyThing) (freq <-chan map[anyThing]int64) {
	cha := make(chan map[anyThing]int64)
	go doneanyThingFreq(cha, inp)
	return cha
}
...
func anyThingDoneFreqAttr(inp <-chan anyThing, attr func(a anyThing) interface{}) (freq <-chan map[interface{}]int64) {
	cha := make(chan map[interface{}]int64)
	go doneanyThingFreqAttr(cha, inp, attr)
	return cha
}

func doneanyThingFreq(out chan<- map[anyThing]int64, inp <-chan anyThing) {
	defer close(out)
...
func doneanyThingFreqAttr(out chan<- map[interface{}]int64, inp <-chan anyThing, attr func(a anyThing) interface{}) {
	defer close(out)
