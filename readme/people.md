# People around go 

Who is who? Here some names of people encountered during the journey.

### The Project Team

- "Robert Griesemer" <gri@golang.org> `gri`
  - Cray Vectorization, Sawzall, Java HotSpot
  - The [`go/types` package]('https://golang.org/pkg/go/types) is a type-checker for Go programs, designed by Robert Griesemer.
    See also: [`github.com/golang/example/gotypes/...`](github.com/golang/example/tree/master/gotypes/)

- "Rob 'Commander' Pike" <r@google.com> - [@robpike](https://github.com/robpike) -
[@ Google+](https://plus.google.com/101960720994009339267) -
[@BlogSpot](https://robpike.blogspot.com/)
  - UTF-8, Squeak, NewSqueak, Plan9, Inferno

- "Ken Thompson" <ken@google.com>
  - C, Unix, UTF-8

started the project in late 2007

---
- "Ian Lance Taylor" <iant@golang.org> - [@ianlancetaylor](https://github.com/ianlancetaylor/)
  - e.g. `gccgo`

- "Russ Cox" <rsc@swtch.com> - [@rsc](https://github.com/rsc) - ![rsc](rsc.png)
  - [swtch.com](http://research.swtch.com/)
  - cindex, csearch ...
  - vgo

joined in 2008

---
plus David Symonds, Nigel Tao, Andrew Gerrand, Stephen Ma, and others

- "Andrew Gerrand" <adg@golang.org> - [@adg](https://github.com/adg) - [@enneff](http://twitter.com/enneff)
  - [AndrewGerrand.com](http://andrewgerrand.com/)
  - [Go and the Zen of Python](https://talks.golang.org/2012/zen.slide#1)

---
### Developpers
- "Brian Mills" <bcmills@google.com> - [@bcmills](https://github.com/bcmills)
  - e.g. `sync.Map`

---
### Contributors
- "Daniel Martí" <mvdan@mvdan.cc> - [@mvdan](https://github.com/mvdan)
  - [mvdan.cc](https://mvdan.cc/)

---
### Bloggers
- [Dave Cheney](https://dave.cheney.net/) The acme of foolishness

- [A Walter Schulze](https://awalterschulze.github.io/blog/) TODO: Normalize Name

- [Sam Boyer](https://sdboyer.io/)
- [0xDEADBEEF](https://deadbeef.me/)
- [Matt Farina - Code Engineered](https://codeengineered.com/)

---
[Back to overview](overview.md)
