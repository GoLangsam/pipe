...
// before close.
func anyThingPipeAdjust(inp <-chan anyThing, sizes ...int) (out <-chan anyThing) {
	cha := make(chan anyThing)
	go pipeanyThingProxy(anyThingSendProxy(cha, sizes...), inp)
	return cha
}

func pipeanyThingProxy(out chan<- anyThing, inp <-chan anyThing) {
	defer close(out)
	for i := range inp {
		out <- i
	}
...
func anyThingTubeAdjust(sizes ...int) (tube func(inp <-chan anyThing) (out <-chan anyThing)) {

	return func(inp <-chan anyThing) (out <-chan anyThing) {
		return anyThingPipeAdjust(inp, sizes...)
	}
}
...
func anyThingSendProxy(out chan<- anyThing, sizes ...int) chan<- anyThing {
	cap, que := sendanyThingProxySizes(sizes...)
	cha := make(chan anyThing, cap)
	go pipeanyThingAdjust(out, cha, que)
	return cha
}
...
func pipeanyThingAdjust(out chan<- anyThing, inp <-chan anyThing, QUE int) {
	defer close(out)
